"""
Script de limpieza para el dataset de VENTAS (Clasificaciones).

Flujo del script:
1. Se carga el archivo Excel original.
2. Se aplica una limpieza general (tipos de datos, columnas y filas).
3. Se aplica una limpieza especifica del dataset de Ventas.
4. Se agregan columnas nuevas a partir de la columna 'Origen'.
5. Se muestra el DataFrame final ya purificado (df_ventas).
"""
...
# --------------------------------------------------------------------------
# FUNCIONES DE LIMPIEZA ESPECIFICA DE VENTAS
# --------------------------------------------------------------------------

def limpieza_especifica_ventas(df):
    """
    Elimina columnas particulares del dataset de Ventas que no aportan
    valor para el analisis (columna 'Centro de costes').
    """
    return df.drop(columns=["Centro de costes"], errors="ignore")


def agregar_columnas_tipo_ventas(df):
    """
    A partir de la columna 'Origen', crea dos columnas nuevas:
    - 'tipo'   : lo que aparece antes de los dos puntos (Caso, Contrato, etc.)
    - 'id_tipo': el numero que aparece luego de 'Nº:' al final del texto

    Ejemplo: 'Caso: Configurar impresora - Nº: 16839'
             -> tipo = 'Caso', id_tipo = 16839
    """
    df = df.copy()
    origen = df["Origen"]
    df["tipo"] = origen.str.split(":", n=1).str[0].str.strip()
    df["id_tipo"] = pd.to_numeric(
        origen.str.extract(r"Nº:\s*(\d+)")[0], errors="coerce"
    ).astype("Int64")
    return df
...
df_ventas = cargar_datos(RUTA_ARCHIVO, HOJA)
df_ventas = limpieza_general(df_ventas, columnas_relevantes=COLUMNAS_RELEVANTES_VENTAS)
df_ventas = limpieza_especifica_ventas(df_ventas)
df_ventas = agregar_columnas_tipo_ventas(df_ventas)
