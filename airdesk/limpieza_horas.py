"""
Script de limpieza para el dataset de HORAS (Carga de Tiempo).

Flujo del script:
1. Se carga el archivo Excel original.
2. Se aplica una limpieza general (tipos de datos, columnas y filas).
3. Se aplica una limpieza especifica del dataset de Horas.
4. Se agrega la columna 'id_caso' a partir de la columna 'Origen'.
5. Se muestra el DataFrame final ya purificado (df_horas).
"""
...
# --------------------------------------------------------------------------
RUTA_ARCHIVO = "raw_data/horas.xlsx"
HOJA = "Carga de Tiempo"

# Columnas que se consideran imprescindibles para que una fila sea valida
COLUMNAS_RELEVANTES_HORAS = ["Nº", "Fecha de Inicio", "Origen", "Usuario"]
...
# --------------------------------------------------------------------------
# FUNCIONES DE LIMPIEZA ESPECIFICA DE HORAS
# --------------------------------------------------------------------------

def limpieza_especifica_horas(df):
    """
    Elimina columnas particulares del dataset de Horas que no aportan
    valor para el analisis.
    """
    columnas_a_eliminar = [
        "Estado inicial del origen creado en",
        "Estado final del origen",
        "Estado final del origen cambiado en",
        "Historial de los estados del origen",
    ]
    return df.drop(columns=columnas_a_eliminar, errors="ignore")


def agregar_id_caso(df):
    """
    Crea la columna 'id_caso', que extrae el numero que aparece al final
    de la columna 'Origen' (despues de 'Nº: ').

    Ejemplo: 'Caso: Configurar impresora - Nº: 16839' -> id_caso = 16839
    """
    df = df.copy()
    df["id_caso"] = pd.to_numeric(
        df["Origen"].str.extract(r"Nº:\s*(\d+)")[0], errors="coerce"
    ).astype("Int64")
    return df
...
df_horas = cargar_datos(RUTA_ARCHIVO, HOJA)
df_horas = limpieza_general(df_horas, columnas_relevantes=COLUMNAS_RELEVANTES_HORAS)
df_horas = limpieza_especifica_horas(df_horas)
df_horas = agregar_id_caso(df_horas)
...
df_horas = filtrar_por_rango_fechas(df_horas, "Fecha de Inicio", fecha_inicio, fecha_fin)

print(df_horas.shape)
print(df_horas.dtypes)
print(df_horas.head())
